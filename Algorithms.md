...
## cell state = f(neibourhood state~(t-1)~)
...
- Robin Karp
- Finite automata 状态迁移 lps 去看上一个匹配失败（匹配失败跳转到 lps），然后固定当前 pattern，就是新的 lps
...
Example 1: Input: n = 4, flights = [[0,1,100],[1,2,100],[2,0,100],[1,3,600],[2,3,200]], src = 0, dst = 3, k = 1 Output: 700 Explanation:

The optimal path with at most 1 stop from city 0 to 3 has cost 100 + 600 = 700. Note that the path through cities [0,1,2,3] is cheaper but is invalid because it uses 2 stops.

Input: n = 3, flights = [[0,1,100],[1,2,100],[0,2,500]], src = 0, dst = 2, k = 1 Output: 200 Explanation: The optimal path with at most 1 stop from city 0 to 2 has cost 100 + 100 = 200.
hi