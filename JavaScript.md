...
3. 观察 2 可以发现， `factorial` 和 `factorial_gen` 逻辑是一样的，唯一差别就是参数化了递归的函数， `factorial_gen(factorial)` `=` `factorial` 那么如果我想让 `factorial_gen` 可以 apply 自己,则需要递归的时候把本地也作为参数传入，可以得到 `factorial_iso` , `factorial_iso(factorial_iso)` `=` `factorial`
...
5. 根据 Y combinator 定义 $Y = (\lambda x (x x)) (\lambda x. g (x x))$ 写成对应的 js 为 `const Y = (f => (x => x(x)(x => f(x(x)))))` 已知 `const factorial_gen = (f) => (n) => n===0?1:n*f(n-1)`
